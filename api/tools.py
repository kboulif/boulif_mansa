...
def preprocess(test_account,test_transactions,nb_month=6):
    ...
    for id in set(accounts_kept['id']):
        ...
        df_id['positive_transactions']=df_id['amount'].apply(lambda x:positive_transactions(x))
        df_id['negative_transactions']=df_id['amount'].apply(lambda x:negative_transactions(x))

        df_id = df_id.groupby('date').sum()
        df_id.sort_values(by='date',ascending=False,inplace=True)
        df_id['balance_per_day']= - df_id['amount']
        df_id['balance_per_day'].iloc[0] += accounts_kept[accounts_kept['id']==id].balance[0]
        ...
        for i,date in enumerate(df_id.index[:-1]):
            ...
            if days==1:
                ...
                for j in range(1,days):
                    ...
                    df_id.loc[dt.timedelta(days=j) + date,['amount','positive_transactions','negative_transactions']]=[0,0,0]
        ...
        for i,date in enumerate(df_id.index[:-30]):
            df_id.loc[date,'sum_outgoing_next_30_days']=df_id.iloc[i+1:i+31]['negative_transactions'].sum()
            df_id.loc[date,'sum_income_next_30_days']=df_id.iloc[i+1:i+31]['positive_transactions'].sum()

        for i in range(nb_month):
            df_id['negative_transaction_last_'+str(i+1)+'_month'] = df_id['sum_outgoing_next_30_days'].shift((i+1)*30)
            df_id['positive_transaction_last_'+str(i+1)+'_month'] = df_id['sum_income_next_30_days'].shift((i+1)*30)
...
def prediction_model(df_final, model):
    df_pred= pd.DataFrame(df_final.iloc[-1]).T
    X=df_pred.drop(['id','sum_outgoing_next_30_days','date','amount','positive_transactions','negative_transactions','sum_income_next_30_days'],axis=1)
    X=X.astype('float32')
